...
const Nav = () => { 
  ...
  bottom-0 mt-auto xl:right-[2%] z-50 top-0 w-full xl:w-16 xl:max-w-md xl:h-screen'>
    ...
    xl:justify-center gap-y-10 px-0 md:px-40 xl:px-0 h-[80] xl:h-max py-8 bg-white/10 
    ...
    xl:rounded-full'>
      ...
      <Link className={`${link.path==pathname && 'text-accent'} relative flex items-center group hover:text-accent transition-all duration-300`} href={link.path} key={index}>
        ...
        <div className='text-[12px] leading-none font-semibold capitalize'>
          ...
          {/* traingle */}
        <div className='border-solid bordel-l-white border-l-8 border-y-transparent border-y-[6px] border-r-0 absolute -right-2 '></div>
          </div>
          </div>
        {/* icon */}
        <div>{link.icon}</div>
